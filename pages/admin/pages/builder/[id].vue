...
.mypage-builder
  ...
  div
    template(v-for="(component, index) in userComponents" :key="component._uid")
      ...
      div.d-flex.flex-column.align-center.justify-center(style="height:50px; position:relative;")
        hr(style="border: 1px dashed rgb(var(--v-theme-secondary)); width:100%;")
        sites-builder-add-block(:position="index" @addBlock="addBlock" :blockList="Layouts")
    div.pt-10(v-if="userComponents.length === 0")
      hr(style="border: 1px dashed rgb(var(--v-theme-secondary));")
      sites-builder-add-block(:position="-1" @addBlock="addBlock" :blockList="Layouts")
</template>
<script setup>
