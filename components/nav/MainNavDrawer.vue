...
v-app
  v-navigation-drawer(v-model="drawer")
    v-list.pa-0
      ...
      v-list-item.px-5(
        ...
        :class="$route.name == nav.name ? 'activated' : ''"
      )
        template(v-slot:prepend)
  ...
  v-app-bar.elevation-1.bg-neutralDark
    ...
    v-btn.elevation-0.text-neutral.rounded-lg(
    ...
    v-btn.elevation-0.text-neutral.mr-3(
      @click="toggleTheme",
      variant="text",
    ...
    v-menu
      template(v-slot:activator="{ props }")
        v-btn.elevation-0.text-neutral.bg-transparent(
          icon="mdi-account",
          v-bind="props"
        )
      v-list.pa-0
  ...
  v-dialog(v-model="openSelectLanguage", width="100%", max-width="350px")
    general-dialog(@close="openSelectLanguage=false")
      v-card-text
        .text-h6.mb-4 
          b {{ $t("chooselanguage") }}
        v-list.pa-0
  ...
  v-dialog(v-model="openSelectCompany", width="100%", max-width="350px")
    general-dialog(@close="openSelectCompany=false")
      v-card-text
        .text-h6.mb-4
          b {{ $t("chooseaccount") }}
      v-card-text
