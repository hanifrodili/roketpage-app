...
div
  ...
  general-dialog-type-a(v-model="dialog" :persistent='true')
      ...
      v-form(ref="form" fast-fail)
          ...
          v-row
            v-col.py-0(cols="12" md="4")
              v-text-field.mb-4(v-model="productForm.price" hide-details="auto" variant="outlined" label="Price" density="compact" type="number" placeholder="0" prefix="RM")
            v-col.py-0(cols="12" md="4")
              v-text-field.mb-4(v-model="productForm.weight" hide-details="auto" variant="outlined" label="Weight" density="compact" type="number" placeholder="0" suffix="KG" )
            v-col.py-0(cols="12" md="4")
              v-text-field.mb-4(v-model="productForm.stock" hide-details="auto" variant="outlined" label="Stock" density="compact" type="number" placeholder="0" )
          v-row
            v-col.py-0(cols="12")
    ...
    template( v-slot:action )
      ...
      v-btn( @click="updateProduct" variant="tonal" color="info" :loading="loading") Save
...
const productForm = ref({
  ...
  price: props.product.base_price/100,
  weight: props.product.base_weight/1000,
  stock: props.product.base_stock,
...
const updateProduct = async () => {
  ...
  if (resp.status === 204) {
    snackbar.add({
      type: 'success',
      text: `${productForm.value.name} updated!`
    })
    emits('updateProduct', true)
  }
